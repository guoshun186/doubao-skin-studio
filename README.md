...
- **内置主题**：橘子洲头（深色，取自橘子洲头风景照取色，自带橘子洲头风景照作为背景图）
...
内置主题「橘子洲头」自带橘子洲头风景照作为背景图，启动即生效：

| 主对话页面 | 技能·连接器页面 |
|------------|----------------|
| ![主对话页面](assets/screenshots/chat-home.png) | ![技能·连接器页面](assets/screenshots/skills-page.png) |

背景图以 40% 透明度浮层显示，确保图片可见的同时文字保持清晰可读。
...
├── assets/
│   ├── README.md                 # 内置主题背景图说明
│   ├── juzizhou.webp             # 橘子洲头内置主题背景图（启动时自动加载）
│   └── screenshots/              # 效果预览截图
...
内置主题「橘子洲头」自带背景图，图片文件位于 `assets/juzizhou.webp`。启动时 `src/apply-skin.mjs` 从相对路径读取该图片，转为 base64 dataURL 内嵌到注入的 CSS 中。

- 如需替换背景图：将新图片命名为 `juzizhou.webp` 放入 `assets/` 目录，重新启动即可
- 如需更换格式（如 PNG）：修改 `src/apply-skin.mjs` 中的 MIME 类型（`data:image/webp` → `data:image/png`）
- 如果图片文件不存在：自动回退为 CSS 渐变背景（使用橘子洲头配色），不会报错
